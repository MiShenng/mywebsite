...
\begin{tabularx}{\linewidth}{@{}X r@{}}
\textbf{$\bullet$ Seeing Is Clicking? How Visual and Textual Clickbait Cues in News Video Thumbnails Shape User Engagement on Bilibili} & \textit{Under review} \\
\end{tabularx}
\quad \textit{Li, Zihao.*} \hfill [\href{https://some-link.com}{\faGlobe}]
\begin{itemize}[nosep, leftmargin=2em, itemsep=3pt, label=$\circ$]
\item Clickbait research has focused almost exclusively on textual headlines, yet for video platforms the thumbnail image---not the title---is the primary point of contact that determines whether users click. This study shifts the analytical lens from text to visuals by investigating how specific visual features of news video thumbnails drive user engagement on Bilibili. Drawing on information gap theory and visual framing, we employ computer vision techniques---including facial emotion recognition, color palette analysis, object salience detection, and visual complexity measurement---to extract and quantify visual characteristics from a large-scale dataset of over 54320 news and current affairs video thumbnails.
\item Engagement is operationalized through multiple platform-native metrics (views, likes, danmaku comments, coin-tossing, and favorites). Results reveal that distinct visual features exert divergent effects across engagement dimensions. The findings suggest that visual sensationalism in thumbnails operates through a curvilinear logic---moderate visual intensity attracts attention while excessive exaggeration triggers viewer skepticism. This study contributes a purely visual analytical framework for clickbait research and provides the first large-scale empirical evidence of how thumbnail visuals independently shape news engagement dynamics on a non-Western video platform.
\end{itemize}
